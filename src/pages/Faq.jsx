...
const Faq = () => {
    ...
    const filteredFaqs = newFaqData?.filter((item) => item?.question?.toLowerCase()?.includes(searchQuery?.toLowerCase()))
                    ...
                    <div className="custom-container">
                        <div className="blog-hero-content">
                            <div className="blog-breadcrumb">
                                <span style={{ color: "#E62E2D" }}>FAQ'S</span> <span className="separator">|</span> <span style={{ color: "#FFFFFF" }}>HOME</span>
                            </div>
                            <h1 className="blog-hero-title">Frequently Asked Questions</h1>
                            <p className="blog-hero-subtitle">
                                Find clear answers to the most common questions about our cybersecurity services, compliance solutions and managed security offerings all in one place.                            </p>
                        </div>
                    </div>
                </div>
                            ...
                            newFaqData?.length > 0 ? (
                                <>
                                    <div className="faq-search-wrapper">
                                        <input type='text' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search for answers..." className="faq-search-input" />
                                        <button className='faq-search-btn' type='button'>Search</button>
                                    </div>
